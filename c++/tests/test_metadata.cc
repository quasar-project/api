...
TEST_CASE("Image metadata", "[metadata]") {
  SECTION("EXIF") {
    SECTION("Read") {
      auto const image = data.binary_data("m1-30-10-2024_11-26-41.jpg");
      auto const metadata = quasar::ImageMetadata::from_exif_bytes(
        quasar::span(image).subspan(0, 2'048)
      );
      ...
      REQUIRE(metadata.library_version() == std::array<std::uint16_t, 3> {0, 0, 0});
